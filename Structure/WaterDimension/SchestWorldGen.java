package com.croparia.mod.Structure.WaterDimension;
...
public class SchestWorldGen extends WorldGenerator {
	...
	public boolean generate(World world, Random rand, BlockPos position) {
		...
		Template template = templatemanager.getTemplate(minecraftserver, new ResourceLocation(Reference.MOD_ID+":seafarer_chest"));
		if(template == null)
		{
			...
			template.addBlocksToWorld(world, position.add(0, 1, 0), placementsettings);
	         ...
	         for (Entry<BlockPos, String> entry : map.entrySet())
	         {
	             if ("chest".equals(entry.getValue()))
	             {
	                 ...
	                 {
	                     ((TileEntityChest)tileentity).setLootTable(LootTableHandler.SEAFARER_HOUSE, rand.nextLong());
	                 }
	             }
	         }
	         return true;
