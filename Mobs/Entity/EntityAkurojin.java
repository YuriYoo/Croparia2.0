...
public class EntityAkurojin extends EntityMob
{
    ...
    public EntityAkurojin(World world)
    {
        super(world);
        this.setSize(1, 2);
    }
    ...
    protected void initEntityAI()
    {
        ...
        this.tasks.addTask(4, new EntityAkurojin.AIFireballAttack(this));
    ...
    protected void applyEntityAttributes()
    {
        ...
        this.getEntityAttribute(SharedMonsterAttributes.ATTACK_DAMAGE).setBaseValue(50.0D / 2);
    ...
    public void onDeath(DamageSource cause)
    {
        ...
        if(cause.getTrueSource() instanceof EntityPlayer)
        {
	    	...
	    	this.dropItem(Items.POISONOUS_POTATO, i); 
        }
    }
    ...
    public void onLivingUpdate()
    {
        ...
        if (this.world.isRemote)
        {
            ...
            for (int i = 0; i < 2; ++i)
            {
                this.world.spawnParticle(EnumParticleTypes.LAVA, this.posX + (this.rand.nextDouble() - 0.5D) * (double)this.width, this.posY + this.rand.nextDouble() * (double)this.height, this.posZ + (this.rand.nextDouble() - 0.5D) * (double)this.width, 0.0D, 0.0D, 0.0D, new int[0]);
            }
            this.world.spawnParticle(EnumParticleTypes.FLAME, this.posX + (this.rand.nextDouble() - 0.5D) * (double)this.width, this.posY + this.rand.nextDouble() * (double)this.height, this.posZ + (this.rand.nextDouble() - 0.5D) * (double)this.width, 0.0D, 0.0D, 0.0D, new int[0]);
    ...
    {
        private final EntityAkurojin blaze;
        private int attackStep;
        private int attackTime;

        public AIFireballAttack(EntityAkurojin entityFlying)
        {
