package com.croparia.mod.World.Dimension;
...
public class AirDim {
	...
	public static BlockairPortal portal;
	public static airTrigger block;

	static {
		portal = (BlockairPortal) (new BlockairPortal().setUnlocalizedName("airdim_portal"));
		block = (airTrigger) (new airTrigger().setUnlocalizedName("airdim_trigger"));
	}

	public AirDim() {
	}

	public static DimensionType dtype = DimensionType.register("airdim", "_airdim", DIMID, AirDim.WorldProviderMod.class, false);

	public void load(FMLInitializationEvent event) {

		if (event.getSide() == Side.CLIENT)
			Minecraft.getMinecraft().getRenderItem().getItemModelMesher()
					.register(block, 0, new ModelResourceLocation(Reference.MOD_ID + ":airdim_trigger", "inventory"));
	...
	public void preInit(FMLPreInitializationEvent event) {
		...
		portal.setRegistryName("airdim_portal");
		ForgeRegistries.BLOCKS.register(portal);
		

		block.setRegistryName("airdim_trigger");
					...
					for (int i1 = -2; i1 <= 2; ++i1) {
						for (int j1 = -1; j1 < 3; ++j1) {
							...
							this.worldServerInstance.setBlockState(new BlockPos(k1, l1, i2),
									flag ? ModBlocks.AIR_BLOCK.getDefaultState() : Blocks.AIR.getDefaultState());
						}
					}
				}
				...
				for (k3 = 0; k3 < 4; ++k3) {
					for (l3 = -1; l3 < 4; ++l3) {
						...
						this.worldServerInstance
								.setBlockState(new BlockPos(i4, j4, k4), flag1 ? ModBlocks.AIR_BLOCK.getDefaultState() : iblockstate, 2);
					}
				}
	...
	static class BlockairMod extends Block {

		protected BlockairMod() {
	...
	public static class BlockairPortal extends Block {
		...
		public boolean tryToCreatePortal(World par1World, int par2, int par3, int par4) {
			byte b0 = 0;
			byte b1 = 0;
			if (getBlock(par1World, par2 - 1, par3, par4) == ModBlocks.AIR_BLOCK || getBlock(par1World, par2 + 1, par3, par4) == ModBlocks.AIR_BLOCK) {
				b0 = 1;
			}
			if (getBlock(par1World, par2, par3, par4 - 1) == ModBlocks.AIR_BLOCK || getBlock(par1World, par2, par3, par4 + 1) == ModBlocks.AIR_BLOCK) {
				b1 = 1;
			}
			if (b0 == b1) {
				return false;
				...
				for (l = -1; l <= 2; ++l) {
					for (i1 = -1; i1 <= 3; ++i1) {
						...
						if (l != -1 && l != 2 || i1 != -1 && i1 != 3) {
							...
							if (flag) {
								if (j1 != ModBlocks.AIR_BLOCK) {
									return false;
								}
							}
							/*
							 * else if (j1 != 0 && j1 !=
							 * Main.Tutorialair.blockID) { return false; }
							 */
						}
					}
				}
		...
		public void neighborChanged(IBlockState state, World par1World, BlockPos pos, Block neighborBlock, BlockPos fromPos) {
			...
			if (getBlock(par1World, par2, i1 - 1, par4) != ModBlocks.AIR_BLOCK) {
				...
				if (j1 == 3 && getBlock(par1World, par2, i1 + j1, par4) == ModBlocks.AIR_BLOCK) {
					...
					if (flag && flag1) {
						...
						if ((getBlock(par1World, par2 + b0, par3, par4 + b1) != ModBlocks.AIR_BLOCK || getBlock(par1World, par2 - b0, par3, par4 - b1) != this)
								&& (getBlock(par1World, par2 - b0, par3, par4 - b1) != ModBlocks.AIR_BLOCK || getBlock(par1World, par2 + b0, par3, par4
										+ b1) != this)) {
							par1World.setBlockToAir(new BlockPos(par2, par3, par4));
						}
					}
	...
	public static class airTrigger extends Item {
		public airTrigger() {
			super();
			this.maxStackSize = 1;
			setMaxDamage(64);
	...
	public static class ChunkProviderModded implements IChunkGenerator {
		protected static final IBlockState field_185982_a = Blocks.AIR.getDefaultState();
		...
		private IBlockState oceanBlock = ModBlocks.CLOUD.getDefaultState();
		...
		protected static final IBlockState GRAVEL = ModBlocks.CLOUD.getDefaultState();
		private final WorldGenerator quartzGen = new WorldGenMinable(Blocks.QUARTZ_BLOCK.getDefaultState(), 14, BlockMatcher.forBlock(ModBlocks.CLOUD));
		...
		public void populate(int x, int z) {
			...
			if (Biome != Biomes.DESERT && Biome != Biomes.DESERT_HILLS && this.settings.useWaterLakes && !flag
				...
				if (net.minecraftforge.event.terraingen.TerrainGen.populate(this, this.world, this.rand, x, z, flag,
					...
					(new WorldGenLakes(Blocks.AIR)).generate(this.world, this.rand, blockpos.add(i1, j1, k1));
				}
			...
			for (int j1 = 255; j1 >= 0; --j1) {
				if (j1 <= rand.nextInt(5)) {
					...
					if (iblockstate2.getMaterial() == Material.AIR) {
						j = -1;
					} else if (iblockstate2.getBlock() == Blocks.MAGMA) {
						if (j == -1) {
							if (k <= 0) {
								iblockstate = AIR;
								iblockstate1 = Blocks.MAGMA.getDefaultState();
	...
	public static class GenLayerBiomesCustom extends GenLayer {
		...
		protected Biome[] allowedBiomes = {AirBiome.biome};
