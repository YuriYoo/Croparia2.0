...
public class EarthDim {
	...
	static {
		portal = (BlockEarthPortal) (new BlockEarthPortal().setUnlocalizedName("earthdim_portal"));
		block = (earthTrigger) (new earthTrigger().setUnlocalizedName("earthdim_trigger"));
	}
	...
	public void load(FMLInitializationEvent event) {
		...
		if (event.getSide() == Side.CLIENT)
			Minecraft.getMinecraft().getRenderItem().getItemModelMesher()
					.register(block, 0, new ModelResourceLocation(Reference.MOD_ID + ":earthdim_trigger", "inventory"));
					...
					for (int i1 = -2; i1 <= 2; ++i1) {
						for (int j1 = -1; j1 < 3; ++j1) {
							...
							this.worldServerInstance.setBlockState(new BlockPos(k1, l1, i2),
									flag ? ModBlocks.EARTH_BLOCK.getDefaultState() : Blocks.AIR.getDefaultState());
						}
					}
				}
				...
				for (k3 = 0; k3 < 4; ++k3) {
					for (l3 = -1; l3 < 4; ++l3) {
						...
						this.worldServerInstance
								.setBlockState(new BlockPos(i4, j4, k4), flag1 ? ModBlocks.EARTH_BLOCK.getDefaultState() : iblockstate, 2);
					}
				}
	...
	public static class BlockEarthPortal extends Block {
		...
		public boolean tryToCreatePortal(World par1World, int par2, int par3, int par4) {
			byte b0 = 0;
			byte b1 = 0;
			if (getBlock(par1World, par2 - 1, par3, par4) == ModBlocks.EARTH_BLOCK || getBlock(par1World, par2 + 1, par3, par4) == ModBlocks.EARTH_BLOCK) {
				b0 = 1;
			}
			if (getBlock(par1World, par2, par3, par4 - 1) == ModBlocks.EARTH_BLOCK || getBlock(par1World, par2, par3, par4 + 1) == ModBlocks.EARTH_BLOCK) {
				b1 = 1;
			}
			if (b0 == b1) {
				return false;
		...
		public void neighborChanged(IBlockState state, World par1World, BlockPos pos, Block neighborBlock, BlockPos fromPos) {
			...
			if (getBlock(par1World, par2, i1 - 1, par4) != ModBlocks.EARTH_BLOCK) {
				...
				if (j1 == 3 && getBlock(par1World, par2, i1 + j1, par4) == ModBlocks.EARTH_BLOCK) {
					...
					if (flag && flag1) {
						...
						if ((getBlock(par1World, par2 + b0, par3, par4 + b1) != ModBlocks.EARTH_BLOCK || getBlock(par1World, par2 - b0, par3, par4 - b1) != this)
								&& (getBlock(par1World, par2 - b0, par3, par4 - b1) != ModBlocks.EARTH_BLOCK || getBlock(par1World, par2 + b0, par3, par4
										+ b1) != this)) {
							par1World.setBlockToAir(new BlockPos(par2, par3, par4));
						}
					}
	...
	public static class ChunkProviderModded implements IChunkGenerator {
		protected static final IBlockState field_185982_a = Blocks.DIRT.getStateFromMeta(2);
		...
		protected static final IBlockState GRAVEL = Blocks.DIRT.getStateFromMeta(1);
		private final WorldGenerator coarseGen = new WorldGenMinable(Blocks.DIRT.getStateFromMeta(1), 30, BlockMatcher.forBlock(Blocks.DIRT));
		private final WorldGenerator quartzGen = new WorldGenMinable(ModBlocks.QUICK_SAND.getDefaultState(), 25, BlockMatcher.forBlock(Blocks.DIRT));
